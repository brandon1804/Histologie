...
function updateLessons() {
    $.ajax({
       ...
       var random = Math.floor(Math.random() * response.length);
            output += "<div class='card2' id=" + response[random].lesson_image_id + ">"
                    + "<div class='row g-0'>"
                    + "<div class='col-md-4'>"
            ...
            for (i = 0; i < response.length; i++) {
                message += "<div class='col-sm-12 col-md-6 col-lg-6 col-xl-4'>"
                        + "<div class='card shadow' style='width: 20rem;' id=" + response[i].lesson_image_id + ">"
                        ...
                        + "<a href='lessonPage.php?id=" + response[i].lesson_image_id + "'class='btn btn-primary'>Start Lesson</a>"
                        + "</div></div></div>";
            }
                ...
                success: function (response) {
                    ...
                    for (i = 0; i < response.length; i++) {
                        message += "<div class='col-sm-12 col-md-6 col-lg-6 col-xl-4'>"
                                ...
                                + "<h5 class='card-title'>" + response[i].title + "</h5>"
                                + "<h6 class='card-subtitle text-muted'>" + response[i].summary + "</h6>"
                                + "<a href='lessonPage.php?id=" + response[i].lesson_id + "'class='btn btn-primary'>Start Lesson</a>"
                                + "</div></div></div>";
                    }
                    $("#lessonsRow").html(message);
                },
